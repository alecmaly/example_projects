...
// Ported from solidity/Main.sol — top-level entry point with all bells.
...
contract Main {
    ...
    function main() public {
        ...
        IShape shape = IShape(new CircleContract(5));
        emit Log(string(abi.encodePacked("Circle.area=", uint2str(shape.area()))));
    ...
    fallback() external payable { emit Log("Fallback function called"); }
    receive() external payable { emit Log("Receive function called"); }
}
